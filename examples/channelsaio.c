...
static void aio_send_cb(meshlink_handle_t *mesh, meshlink_channel_t *channel, void *data, size_t len, void *priv) {
	free(priv);
}
...
static void parse_input(meshlink_handle_t *mesh, char *buf) {
	...
	size_t aiolen = strlen(msg);
	char *aiomsg = malloc(aiolen+1);
	memset(aiomsg, 0, aiolen+1);
	strncpy(aiomsg, msg, aiolen);
	if(!meshlink_channel_aio_send(mesh, channel, aiomsg, aiolen, aio_send_cb, aiomsg)) {
