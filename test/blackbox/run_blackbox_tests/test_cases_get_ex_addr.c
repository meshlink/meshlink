...
/*************************************************************************************/
/**
 * @file      test_cases_get_ex_addr.c -- Execution of specific meshlink black box test cases
 * @see
...
static bool test_steps_mesh_get_address_01(void) {
  ...
  meshlink_handle_t *mesh = meshlink_open("getex_conf.1", "foo", "chat", DEV_CLASS_STATIONARY);
	assert(mesh != NULL);
	if(!mesh) {
	...
	if(addr == NULL) {
		fprintf(stderr, "meshlink_get_external status: %s\n", meshlink_strerror(meshlink_errno));
		return false;
	}
	
	result = meshlink_add_address(mesh, addr);
	assert(result != false);
	if(!result) {
...
static bool test_steps_mesh_get_address_02(void) {
  ...
  meshlink_handle_t *mesh = meshlink_open("getex_conf.2", "foo", "chat", DEV_CLASS_STATIONARY);
	assert(mesh != NULL);
	if(!mesh) {
		fprintf(stderr, "meshlink_open status2: %s\n", meshlink_strerror(meshlink_errno));
		return false;
	...
	result = meshlink_add_address(mesh, ext);
	assert(result != false);
	if(!result) {
		fprintf(stderr, "meshlink_add_address status: %s\n", meshlink_strerror(meshlink_errno));
		return false;
